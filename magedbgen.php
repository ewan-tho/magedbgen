    ...
    if( preg_match('#^create table (.[^\(]+)\(#i', $item, $matches)) {
        ...
        if(preg_match_all('#([a-zA-Z0-9\_\`]+) ([a-zA-Z]+)( ?\(.[^\)]+\))? (UNSIGNED )?(DEFAULT )?(NOT )?NULL( .[^,]+)?#i', $content, $matches)) {
            ...
            foreach ($matches[0] as $key => $match) {
                ...
                $function[] = $tab2 . '\'' . $columnName . '\',';
                $function[] = $tab2 . $columnType . ',';
                $function[] = $tab2 . (empty($columnSize) ? 'null' : $columnSize) . ',';
                if (empty($columnOptions)) {
                    $function[] = $tab2 . '[],';
                } else {
                    $function[] = $tab2 . '[';
                    $function[] = $tab2 . implode(",\n$tab2", $columnOptions);
                    $function[] = $tab2 . '],';
                }
                $function[] = $tab2 . (empty($columnComment) ? '\'\'' : '\'' . $columnComment . '\'');
                $function[] = TAB . ')';
             }
